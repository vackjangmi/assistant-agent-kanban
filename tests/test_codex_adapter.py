...
def test_codex_adapter_builds_exec_command(monkeypatch, tmp_path):
    ...
    assert command[:6] == ["codex", "--ask-for-approval", "never", "--sandbox", "workspace-write", "exec"]
...
def test_codex_adapter_reuses_session_id(monkeypatch, tmp_path):
    ...
    command = cast(list[str], recorded["command"])
    assert command[:8] == ["codex", "--ask-for-approval", "never", "--sandbox", "workspace-write", "exec", "resume", "thread-existing"]
