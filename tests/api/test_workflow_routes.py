...
def test_api_blocks_parallel_local_human_verification_for_same_target_repo(configured_paths):
    ...
    with TestClient(app) as client:
        ...
        returned = client.post(f"/api/tasks/{first.metadata.task_id}/return-verification")
        second_retry = client.post(f"/api/tasks/{second.metadata.task_id}/start-verification")
    ...
    assert returned.json()["state"] == TaskState.COMPLETED_REVIEWS.value
    assert second_retry.status_code == 200
    assert second_retry.json()["state"] == TaskState.HUMAN_VERIFYING.value
    assert scanner.find_task(first.metadata.task_id).state == TaskState.COMPLETED_REVIEWS
    assert scanner.find_task(second.metadata.task_id).state == TaskState.HUMAN_VERIFYING
    ...
    assert current_branch == f"review/{second.metadata.task_id.lower()}"
