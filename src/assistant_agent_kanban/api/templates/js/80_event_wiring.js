    ...
    cancelComposerButton.addEventListener('click', () => { clearMessages(); navigateToBoardPhase(activeBoardPhase || 'plan', { replace: true }); });
    ...
    confirmDirtyTargetConfirmationButton.addEventListener('click', confirmDirtyTargetVerificationStart);
    ...
    requestFollowUpWorkButton.addEventListener('click', () => openFollowUpRequestDraft().catch((error) => { taskModalError.hidden = false; taskModalError.textContent = error.message; }));
