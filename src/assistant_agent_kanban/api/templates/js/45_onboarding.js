  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    function getOnboardingText(key) {
      ...
      if (isUserAdmin) {
        ...
        if (key === 'onboardingStep3Title') actualKey = 'onboardingStep3TitleNonAdmin';
        if (key === 'onboardingStep3Desc') actualKey = 'onboardingStep3DescNonAdmin';
        if (key === 'onboardingStep4Title') actualKey = 'onboardingStep4TitleNonAdmin';
        if (key === 'onboardingStep4Desc') actualKey = 'onboardingStep4DescNonAdmin';
        if (key === 'onboardingStep5Title') actualKey = 'onboardingStep5TitleNonAdmin';
        if (key === 'onboardingStep5Desc') actualKey = 'onboardingStep5DescNonAdmin';
      }
      ...
      const localTable = fallbacks[lang] || fallbacks.EN;
      const localEN = fallbacks.EN;

      if (window.settingsTranslations) {
        const table = window.settingsTranslations[lang] || window.settingsTranslations.EN;
        const globalEN = window.settingsTranslations.EN;

        // 1. Try global table for the actualKey
        if (table[actualKey]) return table[actualKey];
        if (globalEN[actualKey]) return globalEN[actualKey];

        // 2. Try local table for the actualKey
        if (localTable[actualKey]) return localTable[actualKey];
        if (localEN[actualKey]) return localEN[actualKey];

        // 3. Try global table for the base key
        if (table[key]) return table[key];
        if (globalEN[key]) return globalEN[key];

        // 4. Try local table for the base key
        if (localTable[key]) return localTable[key];
        if (localEN[key]) return localEN[key];

        return '';
      }

      return localTable[actualKey] || localEN[actualKey] || localTable[key] || localEN[key] || '';
    }
    ...
    function applyTranslations() {
      ...
      skipBtn.setAttribute('aria-label', getOnboardingText('onboardingSkip'));

      // Steps texts
      for (let i = 1; i <= totalSteps; i++) {
        const titleEl = document.getElementById(`onboarding-title-${i}`);
        const descEl = document.getElementById(`onboarding-desc-${i}`);
        const stepEl = overlay.querySelector(`.onboarding-step[data-step="${i}"]`);
        const iconEl = stepEl ? stepEl.querySelector('.onboarding-icon') : null;
        ...
        if (iconEl) {
          iconEl.textContent = getOnboardingIcon(i);
        }
      }
    ...
    function updateOnboardingState(step, forceExpand = true) {
      ...
      if (isUserAdmin) {
        ...
        else if (step === 3) positionStep = 4; // New Request
        else if (step === 4) positionStep = 5; // Target Project
        else if (step === 5) positionStep = 6; // Draft Refinement
      }
      card.className = `onboarding-card step-${positionStep}`;
        ...
        if (step === 2) {
          ...
          if (window.openSettingsModal) {
            window.openSettingsModal().then(() => {
              // 2. Select git tab
              if (window.setSettingsTab) {
                window.setSettingsTab('git');
              }
              // 3. Highlight the Git tab.
              const tabGit = document.getElementById('settings-tab-git');
              if (tabGit) tabGit.classList.add('onboarding-target-highlight');

              // Robust multi-phase scroll enforcement
              const enforceScrollTop = () => {
                const settingsScrollBody = document.querySelector('#settings-modal .modal-scroll-body');
                if (settingsScrollBody) {
                  settingsScrollBody.scrollTop = 0;
              ...
              // 4. Create pointer badges
              setTimeout(() => {
                const gitUsername = document.getElementById('git_token_username');
                const gitToken = document.getElementById('git_token');
                const tabGeneral = document.getElementById('settings-tab-general');

                if (gitUsername) {
                  createPointerBadge(gitUsername, getOnboardingText('badgeGitUsername'), 'bottom', true);
                }
                if (gitToken) {
                  createPointerBadge(gitToken, getOnboardingText('badgeGitToken'), 'bottom', true);
                }
                if (tabGeneral) {
                  createPointerBadge(tabGeneral, getOnboardingText('badgePreferences'), 'bottom', true);
                }
              }, 100);
          ...
          }).catch(e => console.error('Failed to open request composer in step 7:', e));
        }
      } else {
        // NON-ADMIN SPECIFIC STEPS
        if (step === 2) {
          // Step 2: Git Credentials & Preferences
          // 1. Open settings
          if (window.openSettingsModal) {
            window.openSettingsModal().then(() => {
              // 2. Select git tab
          ...
          // Step 4: Target Project Selection
          // 1. Open request composer modal if not open
          const reqModal = document.getElementById('request-modal');
          const openPromise = (reqModal && reqModal.hidden)
            ? new Promise(resolve => {
          ...
          openPromise.then(() => {
            // Scroll the modal to show the target repo field
            const scrollToTargetRepo = () => {
              const requestScrollBody = document.querySelector('#request-modal .modal-scroll-body');
              if (requestScrollBody) {
                requestScrollBody.scrollTop = 0;
            ...
            // Highlight target repo and base branch fields
            const targetRepoField = document.getElementById('target_repo');
            const baseBranchField = document.getElementById('base_branch');
            if (targetRepoField) {
              const targetRepoGroup = targetRepoField.closest('.field');
          ...
          }).catch(e => console.error('Failed to open request composer in step 4:', e));
        }
        else if (step === 5) {
          // Step 5: Draft with Assistant
          // 1. Open request composer modal if not open
          const reqModal = document.getElementById('request-modal');
          const openPromise = (reqModal && reqModal.hidden)
            ? new Promise(resolve => {
    ...
    function browserOnboardingCompleted() {
      try {
        return localStorage.getItem('kanban_onboarding_completed') === 'true';
      } catch (_error) {
        return false;
      }
    }
    ...
    function updateOnboardingStepsUI() {
      // Dynamic visibility of dots based on totalSteps
      overlay.querySelectorAll('.onboarding-dot').forEach(dot => {
        const dotNum = parseInt(dot.getAttribute('data-step-dot'), 10);
        dot.style.display = dotNum > totalSteps ? 'none' : '';
      });
      
      // Dynamic visibility of panels based on totalSteps
      overlay.querySelectorAll('.onboarding-step').forEach(panel => {
        const stepNum = parseInt(panel.getAttribute('data-step'), 10);
    ...
    async function saveOnboardingCompleted() {
      if (onboardingStateSource === 'user') {
        const response = await fetch('/api/auth/onboarding', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ completed: true, version: onboardingVersion }),
        });
        if (!response.ok) {
          const data = await response.json().catch(() => ({}));
          throw new Error(data.detail || 'Failed to save onboarding state.');
        }
        try {
    ...
    function finishOnboarding() {
      ...
      overlay.classList.remove('minimal-backdrop');
      overlay.hidden = true;
      saveOnboardingCompleted().catch((error) => console.warn(error.message || 'Failed to save onboarding state.'));
    }
    ...
    loadOnboardingCompleted().then((isOnboardingCompleted) => {
