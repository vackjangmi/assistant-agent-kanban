...
def parse_request_markdown(content: str) -> ParsedRequest:
    ...
    title = metadata.get("title") if isinstance(metadata.get("title"), str) else _extract_title(body)
    ...
    return ParsedRequest(title=title, body=body, target_repo_root=target_repo_root, base_branch=base_branch)
