...
- Large local change sets show a capped file preview while preserving the full dirty file count
- Local human verification uses the target repo worktree directly, so only one human verification can be active per target repo
